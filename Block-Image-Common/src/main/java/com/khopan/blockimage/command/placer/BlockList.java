...
public class BlockList {
	...
	public static List<BlockEntry> get() {
		...
		while(iterator.hasNext()) {
			...
			if(!Block.isShapeFullBlock(shape) || !state.isSolidRender(EmptyBlockGetter.INSTANCE, BlockPos.ZERO) || block.hasDynamicShape() || state.getLightEmission() > 0 || block instanceof CoralBlock || block instanceof LeavesBlock) {
				continue;
			}
